...
const deputes = JSON.parse(
  readFileSync(path.join(ROOT, "public/deputes-17.json"), "utf-8"),
);
...
for (let i = 0; i < deputes.length; i += BATCH) {
  ...
  process.stdout.write(
    `  ${Math.min(i + BATCH, deputes.length)}/${deputes.length}\r`,
  );
}
console.log(`  ✓ ${deputes.length} députés insérés`);

console.log("→ Lecture scrutins-17.json…");
const scrutins = JSON.parse(
  readFileSync(path.join(ROOT, "public/scrutins-17.json"), "utf-8"),
);
...
for (let i = 0; i < scrutins.length; i += 100) {
  ...
  process.stdout.write(
    `  ${Math.min(i + 100, scrutins.length)}/${scrutins.length}\r`,
  );
}
