...
async function download(url, dest, retries = 5) {
  for (let i = 0; i < retries; i++) {
    try {
      log(
        `⬇️ Tentative ${i + 1} : Téléchargement de ${url.split("/").pop()}...`,
      );
...
function parseScrutin(s) {
  ...
  const sortLabel = String(
    scrutin?.sort?.libelle ?? scrutin?.sort ?? "Non renseigné",
  );
  return {
    numero: String(scrutin?.numero ?? ""),
    date: scrutin?.dateScrutin ?? "",
    sort: sortLabel,
    approuve: sortLabel.toLowerCase().includes("adopt"),
    pours:
      scrutin?.ventilationVotes?.lesGroupes?.groupe
        ?.map((g) => g?.vote?.decompteNominatif?.pours?.votant ?? [])
        .flat() || [],
  };
}
...
async function main() {
  try {
    ...
    const deputes = _findJsonFilesSync(SOURCES.deputes.outDir)
      .map((f) => {
        try {
          return JSON.parse(readFileSync(f, "utf-8"))?.acteur;
        } catch {
          return null;
        }
      })
      ...
      .map((a) => parseDepute(a));
    writeFileSync(
      SOURCES.deputes.outJson,
      JSON.stringify(deputes, null, 2),
      "utf8",
    );
    ...
    const scrutins = _findJsonFilesSync(SOURCES.scrutins.outDir)
      ...
      .filter(Boolean);
    writeFileSync(
      SOURCES.scrutins.outJson,
      JSON.stringify(scrutins, null, 2),
      "utf8",
    );
