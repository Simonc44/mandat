...
export const Route = createFileRoute("/scrutin/$numero")({
  ...
  head: ({ params }) => ({
    meta: createSeoMeta({
      ...
      description: `Détail du scrutin n°${params.numero} : résultat global, votes par groupe politique, position de chaque député·e.`,
  ...
  notFoundComponent: () => (
    <div className="container-app py-24 text-center animate-fade-up">
      ...
      <Link to="/scrutins" className="btn-primary inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-sm">← Tous les scrutins</Link>
    </div>
  ),
...
function ScrutinPage() {
  ...
  const byGroup = useMemo(() => {
    ...
    return Array.from(m.entries())
      .map(([g, c]) => ({ organeRef: g, positionMajoritaire: "", ...c, total: c.pour + c.contre + c.abstentions + c.nonVotants }))
      .sort((a, b) => b.total - a.total);
  }, [meta.groupes, votes]);
  ...
  const isAdopte = meta.isAdopte ?? /adopt/i.test(sort);
      ...
      <div className="container-app py-12">
        ...
        <div className="mb-10 max-w-4xl animate-fade-up">
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mb-3 uppercase tracking-wider">
            ...
            {meta.legislature && <><span aria-hidden="true">·</span><span>{meta.legislature}e législature</span></>}
          </div>

          <h1 className="font-display text-3xl md:text-4xl leading-tight mb-5">{titre.charAt(0).toUpperCase() + titre.slice(1)}</h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* Badge résultat */}
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold glass"
              style={{
                color: isAdopte ? "var(--color-pour)" : "var(--color-contre)",
                borderColor: isAdopte ? "color-mix(in oklch, var(--color-pour) 30%, transparent)" : "color-mix(in oklch, var(--color-contre) 30%, transparent)",
                backgroundColor: isAdopte ? "color-mix(in oklch, var(--color-pour) 8%, transparent)" : "color-mix(in oklch, var(--color-contre) 8%, transparent)",
              }}
              role="status">
            ...
            {/* Tags */}
            {meta.tags?.map(tag => (
              <span key={tag} className="px-3 py-1 rounded-full text-xs glass text-muted-foreground">{sanitizeText(tag)}</span>
            ))}

            {/* Lien officiel */}
            {meta.url_institution && (
              <a href={meta.url_institution} target="_blank" rel="noreferrer noopener"
        ...
        {/* ── RÉSULTAT GLOBAL ── */}
        <section className="mb-10 p-6 rounded-3xl card-glass animate-fade-up" style={{ animationDelay: "80ms" }} aria-labelledby="result-heading">
          <h2 id="result-heading" className="font-display text-xl mb-5 flex items-center gap-2">
            Résultat global
            <span className="text-sm font-sans font-normal text-muted-foreground">
              ({(pFinal + cFinal + aFinal).toLocaleString("fr-FR")} votants exprimés)
            </span>
          </h2>
        ...
        {byGroup.length > 0 && (
          <section className="mb-10 animate-fade-up" style={{ animationDelay: "160ms" }} aria-labelledby="groups-heading">
            <h2 id="groups-heading" className="font-display text-xl mb-2">Par groupe politique</h2>
            <p className="text-xs text-muted-foreground mb-4">Cliquez sur un groupe pour filtrer la liste.</p>
                  ...
                  <button key={g.organeRef} onClick={() => setFilter(f => ({ ...f, groupe: f.groupe === g.organeRef ? "" : g.organeRef }))}
                    ...
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-sm text-muted-foreground">{groupeMeta(g.organeRef).nom}</span>
                      ...
                      <span className="text-xs text-muted-foreground shrink-0">{g.total} député·es</span>
                    </div>
                    {/* Barre groupe */}
          ...
          <section aria-labelledby="deputies-heading">
            <div className="flex items-end justify-between flex-wrap gap-3 mb-4">
              <h2 id="deputies-heading" className="font-display text-xl">
                ...
                {filter.groupe && <span className="text-base font-sans text-muted-foreground">— {filter.groupe}</span>}
              ...
              <div className="flex flex-wrap gap-1.5" role="group" aria-label="Filtrer par position">
                {([["all","Tous"],["pour","Pour"],["contre","Contre"],["abstention","Abstention"],["nonVotant","Absent"]] as const).map(([k, label]) => (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 animate-stagger">
              {filteredVotes.slice(0, 300).map((v, i) => {
                ...
                return (
                  <Link key={`${v.parlementaire_slug}-${i}`}
                    to="/depute/$slug" params={{ slug }}
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-sm font-medium truncate block group-hover:text-primary transition-colors">{nom || slug}</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: positionColor(v.position) }}>{positionLabel(v.position)}</span>
                    </div>
            ...
            {filteredVotes.length > 300 && (
              <p className="text-xs text-muted-foreground text-center mt-6 py-4 border-t border-border/40">
                Affichage de 300 sur {filteredVotes.length}. Filtrez par groupe ou position pour affiner.
              </p>
            )}
        ...
        {votes.length === 0 && (
          <div className="py-10 text-center glass rounded-3xl border border-border/50">
            <p className="text-muted-foreground mb-3">Les votes nominatifs ne sont pas encore disponibles pour ce scrutin.</p>
            {meta.url_institution && (
              <a href={meta.url_institution} target="_blank" rel="noreferrer noopener" className="text-sm text-primary hover:underline">
...
function AnimatedResultBar({ pour, contre, abstention }: { pour: number; contre: number; abstention: number }) {
  ...
  return (
    <div className="flex h-5 rounded-full overflow-hidden bg-muted/50" role="img"
...
function ResultCell({ label, value, pct, color }: { label: string; value: number; pct: number; color: string }) {
  return (
    <div className="p-4 rounded-2xl glass border border-border/40 text-center"
      style={{ borderColor: `color-mix(in oklch, ${color} 20%, transparent)`, backgroundColor: `color-mix(in oklch, ${color} 5%, transparent)` }}>
      <div className="font-display text-3xl md:text-4xl mb-1" style={{ color }}>{value.toLocaleString("fr-FR")}</div>
...
function DeputeAvatar({ nom, idAn, position }: { nom: string; idAn?: string; position: VotePosition }) {
  ...
  if ((!src17 || err17) && (!src16 || err16)) {
    return (
      <div className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 border-2"
        style={{ backgroundColor: `color-mix(in oklch, ${color} 15%, var(--color-muted))`, borderColor: color, color }}
  ...
  return (
    <div className="relative w-8 h-8 shrink-0">
      {!err17 && src17 ? (
        <img src={src17} alt="" aria-hidden="true" className="w-8 h-8 rounded-full object-cover" loading="lazy" onError={() => setErr17(true)} />
      ) : (
        <img src={src16} alt="" aria-hidden="true" className="w-8 h-8 rounded-full object-cover" loading="lazy" onError={() => setErr16(true)} />
      )}
      <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border border-card"
        style={{ backgroundColor: color }} title={positionLabel(position)} aria-hidden="true" />
    </div>
  );
}
