...
// Page de détail d'un scrutin avec SEO dynamique, JSON-LD, animations,
// visualisation par groupe, protection XSS.
...
export const Route = createFileRoute("/scrutin/$numero")({
  ...
  head: ({ params }) => ({
    meta: createSeoMeta({
      ...
      description: `Détail du scrutin n°${params.numero} à l'Assemblée nationale : résultat global, votes par groupe politique, position nominative de chaque député·e.`,
  ...
  notFoundComponent: () => (
    <div className="container-app py-24 text-center animate-fade-up">
      ...
      <Link to="/scrutins" className="text-primary hover:underline">
        ← Tous les scrutins
      </Link>
    </div>
  ),
...
function ScrutinPage() {
  ...
  const deputeBySlug = useMemo(
    () => new Map(deputes.map((d) => [d.slug, d])),
    [deputes]
  );
    ...
    return votes.filter((v) => {
      ...
      if (filter.pos !== "all") {
        if (filter.pos === "nonVotant") {
          if (
            v.position !== "nonVotant" &&
            v.position !== "nonVotantVolontaire"
          )
            return false;
  ...
  const titre = sanitizeText(meta.titre);
    ...
    <>
      ...
      <div className="container-app py-12">
        ...
        <div className="mb-10 max-w-4xl animate-fade-up">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3 uppercase tracking-wider flex-wrap">
          ...
          <div
            ...
            style={{
              ...
              backgroundColor: `color-mix(in oklch, ${
                isAdopted ? "var(--color-pour)" : "var(--color-contre)"
              } 14%, transparent)`,
            }}
            role="status"
            aria-label={`Résultat : texte ${sort}`}
          >
            {isAdopted ? "✓" : "✗"} Texte {sort}
          </div>
          ...
          {meta.url_institution && (
            <a
              ...
              className="ml-3 inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-primary border border-border hover:border-primary/40 rounded-lg px-3 py-1.5 transition-colors"
            >
              Dossier officiel AN ↗
            </a>
          )}
        ...
        {/* Résultat global */}
        <section
          className="mb-10 p-6 rounded-2xl bg-card border border-border animate-fade-up"
          style={{ animationDelay: "80ms" }}
          aria-labelledby="result-heading"
        >
          <h2 id="result-heading" className="font-display text-xl mb-4">
            Résultat global
          </h2>
          <div className="flex h-4 rounded-full overflow-hidden bg-muted mb-4" role="img" aria-label={`${p} pour, ${c} contre, ${a} abstentions`}>
            <div
              className="result-bar-segment"
              style={{ width: `${(p / total) * 100}%`, backgroundColor: "var(--color-pour)" }}
            />
            <div
              className="result-bar-segment"
              style={{ width: `${(c / total) * 100}%`, backgroundColor: "var(--color-contre)" }}
            />
            <div
              className="result-bar-segment"
              style={{ width: `${(a / total) * 100}%`, backgroundColor: "var(--color-abstention)" }}
            />
          </div>
        ...
        >
          <h2 id="groups-heading" className="font-display text-xl mb-4">
        ...
        <section aria-labelledby="deputies-heading">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 animate-stagger">
            {filteredVotes.slice(0, 300).map((v, i) => {
              const dep = deputeBySlug.get(v.parlementaire_slug);
              const name = dep
                ? `${sanitizeText(dep.prenom)} ${sanitizeText(dep.nom_de_famille)}`
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-2 p-2.5 rounded-lg bg-card border border-border hover:border-primary/40 transition-colors animate-fade-up group"
...
function Seg({ n, t, c }: { n: number; t: number; c: string }) {
  ...
  return (
    <div
      className="result-bar-segment"
      style={{ width: `${(n / Math.max(1, t)) * 100}%`, backgroundColor: c }}
    />
  );
}
