...
const THEMES = [
  ...
  },
  {
    key: "education",
    label: "🏫 Éducation",
    terms: ["education", "enseignement", "école", "université", "recherche"],
  },
  {
    key: "sante",
    label: "🏥 Santé",
    terms: ["santé", "médecin", "hôpital", "pharmacie", "médicament"],
  },
] as const;
...
function ScrutinPage() {
  ...
  const byGroup = useMemo(() => {
    ...
    return Array.from(m.entries())
      .map(([g, c]) => ({
        organeRef: g,
        nom: groupeMeta(g).nom,
        couleur: groupeMeta(g).couleur,
        positionMajoritaire: "",
        ...c,
      ...
      <div className="container-app py-12">
        ...
        <div className="mb-8 max-w-4xl animate-fade-up">
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mb-3 uppercase tracking-wider">
            ...
            {themeObj && (
              <span
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full glass border border-border/40 text-muted-foreground"
                style={{ fontSize: 10 }}
              >
                {themeObj.label}
              </span>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span
              ...
              role="status"
            >
              <span aria-hidden="true">{isAdopte ? "✓" : "✗"}</span>Texte{" "}
          ...
          >
            ...
            <div className="space-y-3">
              {byGroup.map((g, i) => {
                ...
                const posMaj =
                  g.pour >= g.contre && g.pour >= g.abstentions
                    ? "pour"
                    ...
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <span
                          ...
                          style={{
                            color: posMajColor,
                            backgroundColor: `color-mix(in oklch, ${posMajColor} 12%, transparent)`,
                          }}
                        >
                          {posMajLabel}
                        </span>
        ...
        {votes.length > 0 && (
          <section aria-labelledby="deputies-heading">
            <div className="flex items-end justify-between flex-wrap gap-4 mb-6">
              ...
              >
                {(
                  ["all", "pour", "contre", "abstention", "nonVotant"] as const
                ).map((k) => (
                  <button
                    key={k}
                    onClick={() =>
                  ...
                  >
                    {k === "all"
                      ? `Tous (${votes.length})`
                      : k === "pour"
                        ? `Pour (${votesByPosition.pour.length})`
                        : k === "contre"
                          ? `Contre (${votesByPosition.contre.length})`
                          : k === "abstention"
                            ? `Abstention (${votesByPosition.abstention.length})`
                            : `Absent (${votesByPosition.absent.length})`}
                  </button>
                ))}
              </div>
            </div>
            ...
            {filter.pos === "all" && filter.groupe === "" ? (
                  ...
                  [
                    {
                      label: "✓ Pour",
                      color: "var(--color-pour)",
                      list: votesByPosition.pour,
                    },
                    {
                      label: "✗ Contre",
                      color: "var(--color-contre)",
                      list: votesByPosition.contre,
                    },
                    {
                      label: "Abstention",
                      color: "var(--color-abstention)",
                      list: votesByPosition.abstention,
                    },
                    {
                      label: "Absent·es",
                      color: "var(--color-absent, oklch(0.7 0 0))",
                      list: votesByPosition.absent,
                    },
                  ] as const
                )
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 animate-stagger">
                ...
                {filteredVotes.length > 300 && (
                  <p className="text-xs text-muted-foreground text-center mt-8 py-6 border-t border-border/30 col-span-full">
                    Affichage de 300 sur {filteredVotes.length}. Filtrez par
                    groupe ou position.
                  </p>
                )}
                {filteredVotes.length === 0 && (
                  <div className="py-16 text-center glass rounded-[2rem] border border-border/50 col-span-full">
