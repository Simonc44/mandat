...
export const Route = createFileRoute("/depute/$slug")({
  ...
  head: ({ params }) => {
    ...
    return {
      meta: createSeoMeta({
        ...
        description: `Historique complet des votes de ${name} à l'Assemblée nationale (17e législature).`,
  ...
  notFoundComponent: () => (
    <div className="container-app py-24 text-center animate-fade-up">
      <div className="text-5xl mb-4" style={{ fontSize: "3rem" }}>🏛</div>
      <h1 className="font-display text-4xl mb-3">Député·e introuvable</h1>
      <p className="text-muted-foreground mb-6">Ce profil n'existe pas ou n'est pas encore indexé.</p>
      <Link to="/deputes" className="btn-primary inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-sm">← Tous les député·es</Link>
    </div>
  ),
...
function DeputePage() {
  ...
  const filteredVotes = useMemo(() => {
    ...
    if (posFilter === "nonVotant") return votes.filter(v => v.position === "nonVotant" || v.position === "nonVotantVolontaire");
    return votes.filter(v => v.position === posFilter);
  }, [votes, posFilter]);

  const personJsonLd = useMemo(() => createPersonSchema({
    name: sanitizeText(d.nom), firstName: sanitizeText(d.prenom),
    lastName: sanitizeText(d.nom_de_famille),
    imageUrl: d.id_an ? photoUrl(d.id_an, 17) : "",
    party: sanitizeText(d.groupe_sigle), region: sanitizeText(d.nom_circo), slug: safeSlug,
  }), [d, safeSlug]);

  const breadcrumbJsonLd = useMemo(() => createBreadcrumbSchema([
    ...
    <>
      ...
      <div className="container-app py-12">
        ...
        <nav aria-label="Fil d'Ariane" className="mb-6 animate-fade-in">
          <Link to="/deputes" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary transition-colors">
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-10 mb-10 animate-fade-up">
          ...
          <div className="relative shrink-0">
            <div className="w-36 h-36 md:w-52 md:h-52 rounded-3xl overflow-hidden bg-muted shadow-xl ring-1 ring-border/50">
              {!imgError17 && photo17 ? (
                <img src={photo17} alt={`Portrait de ${sanitizeText(d.prenom)} ${sanitizeText(d.nom_de_famille)}`}
                  className="w-full h-full object-cover" width={208} height={208} onError={() => setImgError17(true)} />
              ) : !imgError16 && photo16 ? (
                <img src={photo16} alt={`Portrait de ${sanitizeText(d.prenom)} ${sanitizeText(d.nom_de_famille)}`}
                  className="w-full h-full object-cover" width={208} height={208} onError={() => setImgError16(true)} />
              ) : (
                <div className="w-full h-full flex items-center justify-center font-display text-5xl font-semibold"
                  style={{ background: "linear-gradient(135deg, oklch(0.50 0.20 285 / 15%), oklch(0.42 0.22 215 / 25%))", color: "oklch(0.50 0.20 285)" }} aria-hidden="true">
                  {initials}
                </div>
              )}
            </div>
          ...
          <div className="flex-1 min-w-0 pt-2">
            <h1 className="font-display text-4xl md:text-5xl mb-2 leading-tight">
              {sanitizeText(d.prenom)} <span className="font-bold">{sanitizeText(d.nom_de_famille)}</span>
            </h1>
            <p className="text-base text-muted-foreground mb-1">
              Député·e de <strong className="text-foreground">{sanitizeText(d.nom_circo)}</strong>
              {d.num_deptmt ? ` (${d.num_deptmt})` : ""}
              {d.num_circo ? ` · circ. ${d.num_circo}` : ""}
            </p>
            {d.profession && <p className="text-sm text-muted-foreground mb-1">{sanitizeText(d.profession)}</p>}
            {d.mandat_debut && (
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {d.url_an && (
                <a href={d.url_an} target="_blank" rel="noreferrer noopener"
                  ...
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" strokeLinecap="round" strokeLinejoin="round"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                  Fiche officielle AN
                </a>
              )}
              {d.twitter && (
        ...
        {/* ── BARRE PRÉSENCE ── */}
        {stats.total > 0 && (
          <div className="card-glass rounded-2xl p-5 mb-8 animate-fade-up" style={{ animationDelay: "150ms" }}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium">Taux de participation</span>
              <span className="font-display text-2xl" style={{ color: "oklch(0.50 0.20 285)" }}>{stats.presence}%</span>
            </div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {stats.pour + stats.contre + stats.abstention} votes exprimés · {stats.absent} absences · {stats.total} scrutins
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-4 animate-fade-in" style={{ animationDelay: "200ms" }}
          role="group" aria-label="Filtrer les votes par position">
          {([
            ["all", "Tous les votes"],
            ...
            <button key={k} onClick={() => setPosFilter(k as VotePosition | "all")} aria-pressed={posFilter === k}
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 ${
                posFilter === k ? "btn-primary border-transparent" : "glass border-border/50 text-foreground/70 hover:text-foreground"
              }`}>
              {label}
            </button>
          ))}
        ...
        <h2 className="font-display text-2xl mb-4">
          Votes <span className="text-base font-sans text-muted-foreground">({filteredVotes.length})</span>
        </h2>
          ...
          <ul className="space-y-2 animate-stagger" aria-label="Historique des votes">
            {filteredVotes.slice(0, 150).map((v, i) => (
              <li key={`${v.scrutin.numero}-${i}`} className="animate-fade-up" style={{ animationDelay: `${Math.min(i * 20, 300)}ms` }}>
                <Link to="/scrutin/$numero" params={{ numero: v.scrutin.numero }}
                  className="flex items-start gap-3 p-4 rounded-2xl card-glass group border border-border/40">
                  <span className="shrink-0 mt-0.5 px-2.5 py-1 rounded-xl text-xs font-semibold uppercase tracking-wider"
                    style={{ color: positionColor(v.position), backgroundColor: `color-mix(in oklch, ${positionColor(v.position)} 12%, transparent)` }}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-2 text-xs text-muted-foreground mt-1 flex-wrap">
                      {v.scrutin.date && <time dateTime={v.scrutin.date}>{new Date(v.scrutin.date).toLocaleDateString("fr-FR", { day: "numeric", month: "short", year: "numeric" })}</time>}
                      {v.scrutin.sort && (
                  ...
                  <svg className="self-center shrink-0 text-muted-foreground opacity-0 group-hover:opacity-100 transition-all" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m9 18 6-6-6-6"/></svg>
                </Link>
              </li>
            ))}
          </ul>
...
function StatBox({ label, value, color }: { label: string; value: string | number; color: string }) {
  return (
    <div className="stat-box card-glass p-4 rounded-2xl" style={{ borderColor: `color-mix(in oklch, ${color} 20%, transparent)` }}>
