...
export const Route = createFileRoute("/groupes")({
  head: () => ({
    meta: createSeoMeta({
      ...
      description:
        "Analysez la proximité de vote entre les groupes politiques de l'Assemblée nationale. Simulez des coalitions et calculez leur majorité.",
...
function GroupesPage() {
  ...
  const proximite = useMemo(() => {
    ...
    const concordance = new Map<
      string,
      Map<string, { same: number; total: number }>
    >();
    sigles.forEach((a) => {
      concordance.set(a, new Map());
      sigles.forEach((b) => concordance.get(a)!.set(b, { same: 0, total: 0 }));
    });
    for (const s of scrutins) {
      if (!s.groupes || s.groupes.length < 2) continue;
      const positions = new Map<string, string>();
      for (const g of s.groupes) {
        if (g.positionMajoritaire && g.organeRef)
          positions.set(g.organeRef, g.positionMajoritaire);
      }
    ...
    sigles.forEach((a) => {
      ...
      sigles.forEach((b) => {
        ...
        const pct =
          cell && cell.total >= 5
            ? Math.round((cell.same / cell.total) * 100)
            : null;
    ...
    <div className="container-app py-12">
      <div className="mb-10 animate-fade-up">
        ...
        <p className="text-muted-foreground max-w-2xl">
          Proximité de vote entre groupes et simulateur de coalition — basés sur
          les scrutins réels de la 17e législature.
        </p>
      </div>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mb-6">
          Pourcentage de scrutins où deux groupes ont voté dans le même sens.
          Vert = souvent d'accord, rouge = souvent opposés.
        </p>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mb-6">
          Cliquez sur les groupes pour les ajouter à la coalition. La ligne
          rouge marque les 289 sièges de majorité absolue.
        </p>
...
function heatColor(pct: number): string {
  ...
  if (pct >= 70)
    return `color-mix(in oklch, oklch(0.52 0.17 145) ${Math.round(40 + (pct - 70) * 2)}%, transparent)`;
  if (pct >= 40)
    return `color-mix(in oklch, oklch(0.65 0.18 60) ${Math.round(15 + (pct - 40) * 0.8)}%, transparent)`;
  return `color-mix(in oklch, oklch(0.55 0.20 15) ${Math.round(20 + (100 - pct) * 0.5)}%, transparent)`;
}

function ProximiteHeatmap({
  groupes,
  proximite,
}: {
  groupes: GroupeStats[];
  proximite: Map<string, Map<string, number>>;
}) {
  ...
  return (
    <div className="card-glass rounded-[2rem] p-5 overflow-x-auto">
      <table
        ...
        style={{ fontSize: 11 }}
        aria-label="Matrice de proximité entre groupes"
      >
        <thead>
          <tr>
            {/* Coin vide */}
          ...
          {top.map((rowG) => (
            <tr key={rowG.sigle}>
              ...
              <td className="pr-4 py-0.5">
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: rowG.couleur }}
                    aria-hidden="true"
                  />
                </div>
              </td>
              {top.map((colG) => {
                ...
                const pct = isDiag
                  ? 100
                  : (proximite.get(rowG.sigle)?.get(colG.sigle) ?? -1);
                const isHov =
                  hovered &&
                  ((hovered[0] === rowG.sigle && hovered[1] === colG.sigle) ||
                    (hovered[0] === colG.sigle && hovered[1] === rowG.sigle));
                return (
                  <td
                    key={colG.sigle}
                  ...
                  >
                    <div
                      ...
                      onMouseEnter={() =>
                        !isDiag && setHovered([rowG.sigle, colG.sigle])
                      }
                      ...
                      title={
                        isDiag
                          ? rowG.nom
                          : pct >= 0
                            ? `${rowG.sigle} / ${colG.sigle} : ${pct}% de scrutins convergents`
                            : "Données insuffisantes (<5 scrutins communs)"
                      }
                      aria-label={
                        isDiag
                          ? rowG.nom
                          : pct >= 0
                            ? `${rowG.sigle} et ${colG.sigle} : ${pct}%`
                            : "—"
                      }
                    >
        ...
        (() => {
          ...
          const label =
            pct < 0
              ...
              : pct >= 70
                ? "Votes très convergents"
                : pct >= 40
                  ? "Votes modérément convergents"
                  : "Votes souvent opposés";
          return (
            <div
              className="mt-4 px-4 py-3 rounded-2xl glass border border-border/50 text-sm animate-fade-up"
              style={{ maxWidth: 420 }}
            >
              <span className="font-semibold" style={{ color: gA.couleur }}>
                {gA.nom}
              </span>
              <span className="text-muted-foreground mx-2">+</span>
              <span className="font-semibold" style={{ color: gB.couleur }}>
                {gB.nom}
              </span>
...
function CoalitionSimulator({ groupes }: { groupes: GroupeStats[] }) {
  ...
  const toggle = (sigle: string) =>
    setSelected((prev) => {
      ...
      n.has(sigle) ? n.delete(sigle) : n.add(sigle);
      return n;
    });

  const totalSieges = groupes
    .filter((g) => selected.has(g.sigle))
    .reduce((s, g) => s + g.sieges, 0);
  ...
  const seatList: { couleur: string; sigle: string; inCoalition: boolean }[] =
    useMemo(() => {
      ...
      for (const g of groupes) {
        for (let i = 0; i < g.sieges; i++) {
          seats.push({
            couleur: g.couleur,
            sigle: g.sigle,
            inCoalition: selected.has(g.sigle),
          });
        }
      }
      return seats;
    ...
    <div className="card-glass rounded-[2rem] p-6 space-y-6">
      ...
      <div className="w-full overflow-x-auto">
        <svg
          ...
          style={{ minWidth: 320 }}
          role="img"
          aria-label="Hémicycle de l'Assemblée nationale — simulation de coalition"
        >
          {/* Fond arc */}
          <path
            d={`M ${CX - 330} ${CY} A 330 330 0 0 1 ${CX + 330} ${CY}`}
            fill="none"
            stroke="var(--border)"
            strokeWidth="1"
            opacity="0.4"
          ...
          <text
            ...
            textAnchor="middle"
            fontSize="10"
            fill={hasMajorite ? "oklch(0.45 0.18 145)" : "oklch(0.55 0.02 0)"}
            fontWeight="500"
          >
            {hasMajorite
              ? `✓ Majorité absolue (+${totalSieges - MAJORITE_ABSOLUE})`
              : totalSieges === 0
                ? "Sélectionnez des groupes"
                : `${MAJORITE_ABSOLUE - totalSieges} sièges manquants`}
          </text>
        </svg>
      </div>
              ...
              style={
                active
                  ? {
                      ...
                      backgroundColor: `color-mix(in oklch, ${g.couleur} 12%, transparent)`,
                    }
                  : {}
              }
            >
              ...
              <div className="min-w-0 flex-1">
                <div
                  ...
                  style={{ color: active ? g.couleur : "var(--foreground)" }}
                >
                  {g.sigle}
                </div>
