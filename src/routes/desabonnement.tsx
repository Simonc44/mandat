// routes/desabonnement.tsx
// Gestion des abonnements : Google Sign-In côté client + email envoyé à l'API

import { createFileRoute, Link } from "@tanstack/react-router";
import { useState, useEffect, useCallback } from "react";
import {
  ShieldAlert, Check, Trash2, ArrowLeft,
  LogOut, Calendar, ExternalLink, Loader2, Bell,
} from "lucide-react";
...
export const Route = createFileRoute("/desabonnement")({
  head: () => ({
    meta: createSeoMeta({
      ...
      description: "Gérez ou supprimez vos alertes de vote des députés.",
...
// Décoder le JWT Google côté client pour extraire l’email
function decodeGoogleJwt(token: string): UserProfile | null {
  try {
...
function GoogleSignInButton({ onClick, loading }: { onClick: () => void; loading: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="group relative w-full max-w-xs mx-auto flex items-center justify-center gap-3 px-6 py-3.5 rounded-2xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60 disabled:cursor-wait"
      style={{
        background: "linear-gradient(135deg, rgba(255,255,255,0.88) 0%, rgba(255,255,255,0.62) 100%)",
        backdropFilter: "blur(20px) saturate(1.8)",
        WebkitBackdropFilter: "blur(20px) saturate(1.8)",
        border: "1.5px solid rgba(255,255,255,0.75)",
        boxShadow: "0 4px 24px rgba(0,0,0,0.08), 0 1px 4px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)",
      }}
    >
      {/* Reflet */}
      ...
      {loading ? (
        ...
        <svg className="w-5 h-5 shrink-0 relative z-10" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <span className="relative z-10 text-sm font-semibold tracking-tight" style={{ color: "oklch(0.25 0.04 285)" }}>
        {loading ? "Connexion en cours…" : "Continuer avec Google"}
      </span>
      <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
        style={{ background: "linear-gradient(135deg, rgba(66,133,244,0.06) 0%, rgba(52,168,83,0.04) 100%)" }} />
    </button>
  );
}
...
function DesabonnementPage() {
  ...
  const fetchSubscriptions = useCallback(async (email: string) => {
    ...
    try {
      ...
      setSelectedIds(new Set((data as Subscription[]).map((s) => s.id)));
    } catch (e) {
      setMessage({ type: "error", text: e instanceof Error ? e.message : "Erreur inconnue" });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleGoogleCallback = useCallback((response: { credential: string }) => {
    ...
    sessionStorage.setItem("mandat_user", JSON.stringify(profile));
    setUser(profile);
    setMessage(null);
    fetchSubscriptions(profile.email).finally(() => setGoogleLoading(false));
  }, [fetchSubscriptions]);

  // Initialiser Google Sign-In
  useEffect(() => {
    // Restaurer la session
    const saved = sessionStorage.getItem("mandat_user");
    if (saved) {
      try {
    ...
    script.onload = () => {
      if (window.google?.accounts?.id) {
        window.google.accounts.id.initialize({
          ...
          auto_select: false, // évite le FedCM auto qui génère les AbortError
        });
        setGoogleReady(true);
      }
    };
  ...
  }, [clientId, handleGoogleCallback, fetchSubscriptions]);

  const handleGoogleClick = () => {
    if (!googleReady || !window.google?.accounts?.id) {
      setMessage({ type: "error", text: "Google Sign-In non disponible. Rechargez la page." });
      return;
    }
    setGoogleLoading(true);
    // prompt() ouvre le sélecteur de compte Google natif
    window.google.accounts.id.prompt();
    // Si l'utilisateur ferme sans se connecter, stopper le loading après 5s
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      const res = await fetch("/api/delete-subscriptions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user.email, ids: Array.from(selectedIds) }),
      });
      const data = await res.json();
      ...
      setSelectedIds(new Set());
      setMessage({ type: "success", text: `${count} abonnement${count > 1 ? "s" : ""} supprimé${count > 1 ? "s" : ""}. Vous ne recevrez plus d’alertes.` });
    } catch (e) {
      setMessage({ type: "error", text: e instanceof Error ? e.message : "Erreur inconnue." });
    } finally {
  ...
  return (
    <div className="container-app py-12 md:py-20 max-w-2xl mx-auto">
      ...
      <header className="mb-10 text-center">
        ...
        <h1 className="font-display text-3xl sm:text-4xl mb-3 tracking-tight">Gérer mes abonnements</h1>
      ...
      {message && (
        <div className={`mb-6 p-4 rounded-2xl border text-sm flex gap-3 items-start animate-fade-in ${
          message.type === "success"
            ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-700"
            : "bg-destructive/10 border-destructive/20 text-destructive"
        }`}>
          <div className="mt-0.5 shrink-0">{message.type === "success" ? <Check className="w-4 h-4" /> : "⚠️"}</div>
          <p>{message.text}</p>
        </div>
      )}
      ...
      {!user ? (
        <div className="card-glass rounded-[2rem] p-10 border border-border/40 text-center space-y-8">
          <div className="space-y-2">
            ...
            <h2 className="font-display text-xl tracking-tight">Connexion sécurisée</h2>
            <p className="text-xs text-muted-foreground leading-relaxed max-w-sm mx-auto">
              Google vérifie que l'adresse email vous appartient avant d'afficher vos abonnements.
            </p>
          </div>
          ...
          {clientId ? (
            <div className="flex flex-col items-center gap-4">
              ...
              {!googleReady && !googleLoading && (
                <p className="text-[10px] text-muted-foreground">Chargement de Google Sign-In…</p>
              )}
              <p className="text-[10px] text-muted-foreground">Connexion sécurisée · Aucun mot de passe stocké · RGPD</p>
            </div>
          ) : (
            <div className="rounded-2xl p-5 text-sm text-left space-y-3"
              style={{ background: "oklch(0.96 0.03 80 / 50%)", border: "1px solid oklch(0.88 0.06 80 / 50%)", color: "oklch(0.45 0.12 70)" }}>
              <p className="font-semibold">⚙️ Configuration requise</p>
              <p className="text-xs leading-relaxed">
                Ajoutez la variable <code className="px-1.5 py-0.5 rounded-md text-[11px] font-mono" style={{ background: "oklch(0.90 0.06 80 / 40%)" }}>VITE_GOOGLE_CLIENT_ID</code> dans Vercel.
              </p>
        ...
        <div className="space-y-6">
          ...
          <div className="card-glass rounded-2xl p-4 border border-border/40 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              {user.picture ? (
                <img src={user.picture} alt={user.name ?? "Avatar"} className="w-10 h-10 rounded-full border border-primary/20" />
              ) : (
              ...
              <div className="min-w-0">
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Connecté avec Google</div>
            ...
            <div className="flex items-center justify-between">
              ...
              {subscriptions.length > 0 && (
                <button onClick={() => setSelectedIds(
                  selectedIds.size === subscriptions.length ? new Set() : new Set(subscriptions.map((s) => s.id))
            ...
            {loading ? (
              ...
              <div className="py-12 text-center space-y-4">
                ...
                <p className="text-xs text-muted-foreground max-w-xs mx-auto leading-relaxed">
                  Vous ne suivez aucun député avec <strong>{user.email}</strong>.
                </p>
                <Link to="/deputes"
                  className="inline-flex items-center gap-1.5 text-xs text-primary font-semibold hover:underline">
                  Parcourir les député·es <ExternalLink className="w-3.5 h-3.5" />
                </Link>
              </div>
            ) : (
                  ...
                  return (
                    <div key={sub.id}
                      ...
                      className={`flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer select-none ${
                        checked ? "border-primary/40 bg-primary/5" : "border-border/40 hover:border-border/70"
                      }`}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-semibold truncate">{sub.depute_nom}</div>
                      ...
                      <Link to="/depute/$slug" params={{ slug: sub.depute_slug }}
